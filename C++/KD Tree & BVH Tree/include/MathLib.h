...
public:
	...
	void Rotate(const Vector& v, const float a, bool deg = true);
...
{
public:
	Color(float r = 0.0f, float g = 0.0f, float b = 0.0f, float a = 0.0f) : R(r), G(g), B(b), A(a) {};
	~Color();
	float R, G, B, A;
};
...
public:
	...
	Color MyColor;
	float *VertexArray; //vertex/face array, normal, and normal arrays
	float *VertexNormals;
