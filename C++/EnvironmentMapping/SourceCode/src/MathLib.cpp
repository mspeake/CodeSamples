...
**MathLib.cpp
**
**-Implements a 3D math library, including functionality for many
** linear algebra components, and quaternions
********************************************************************/
...
bool Point::SameSide(const Point& p, const LineSegment& ls) const
{
	...
	float DotProd = MathUtils::ClampTo(V1.Dot(V2), 0.0f, TOLERANCE); //in case of very near zero
	if(DotProd >= 0.0f)
		return true;
	else
		return false;
...
float Point::Distance(const AABB& a) const
{
	...
	Point P1(MathUtils::Clamp(X, Min.X, Max.X), MathUtils::Clamp(Y, Min.Y, Max.Y), MathUtils::Clamp(Z, Min.Z, Max.Z));
	return this->Distance(P1);
}
...
bool Point::IsContained(const LineSegment& ls) const
{
	...
	if(this->IsContained(L))
	{
		...
		return MathUtils::InRange(Res, 0.0f, 1.0f); 
	}
	else
		return false;
}
...
bool Point::IsContained(const Sphere& s) const
{
	...
	if(MathUtils::ClampTo(Dist, s.R, TOLERANCE) <= s.R) //consider very close to R to be close enough
		return true;
	else
		return false;
}
...
bool Point::IsContained(const AABB& a) const
{
	...
	if(!MathUtils::InRange(X, Min.X, Max.X)) //X does not lie within the extents, not contained
		return false;

	if(!MathUtils::InRange(Y, Min.Y, Max.Y)) //Y does not lie within the extents, not contained
		return false;

	if(!MathUtils::InRange(Z, Min.Z, Max.Z)) //Z does not lie within the extents, not contained
		return false;
...
bool Vector::IsZeroVector(void) const
{
	...
	else if(!MathUtils::NearZero(Y))
		return false; //Y is not 0, not the zero vector
	else if(!MathUtils::NearZero(Z))
...
bool Vector::IsParallel(const Vector& v) const
{
	...
	if(MathUtils::ClampTo(Res, 1.0f, TOLERANCE) == 1.0f)
...
bool Line::IsParallel(const Line& l) const
{
	...
	else
	{
		...
		if(MathUtils::ClampTo(Res, 1.0f, TOLERANCE) == 1.0f) //close enough to 1
			return true;
		else
			return false;
	}
...
{
	float Res = V.Dot(p.N);
	return MathUtils::NearZero(Res); //parallel if the dot product is 0, or close enough
}
...
float LineSegment::Distance(const LineSegment& ls) const
{
	...
	if(Denom != 0.0f)
		S1 = MathUtils::Clamp((B*E - C*D) / Denom, 0.0f, 1.0f);
	...
	{
		T1 = 0.0f;
		S1 = MathUtils::Clamp(-C / A, 0.0f, 1.0f);
	}
	else if(T1 > 1.0f)
	{
		T1 = 1.0f;
		S1 = MathUtils::Clamp((B - C) / A, 0.0f, 1.0f);
	}
...
bool LineSegment::Intersects(const Plane& p, Intersection& inter) const
{
	...
	if(!MathUtils::InRange(T, 0.0f, 1.0f))
		return false;
...
bool Ray::Intersects(const Plane& p, Intersection& inter) const
{
	...
	if(MathUtils::ClampTo(T, 0.0f, TOLERANCE) < 0.0f)
		return false;
...
Quaternion Quaternion::operator*(const Quaternion& rhs) const
{
	...
	Ret.S = MathUtils::ClampTo(Ret.S, 0.0f, TOLERANCE);
	Ret.V.X = MathUtils::ClampTo(Ret.V.X, 0.0f, TOLERANCE);
	Ret.V.Y = MathUtils::ClampTo(Ret.V.Y, 0.0f, TOLERANCE);
	Ret.V.Z = MathUtils::ClampTo(Ret.V.Z, 0.0f, TOLERANCE);
...
void Range::IntersectWith(const Range& r)
{
	...
	Min = MathUtils::MyMax(A, C);
	Max = MathUtils::MyMin(B, D);
}
//-----Range Class Functions-----//
...
namespace MathUtils
{
	...
	//Clamps "val" to the given range(low - high)
	...
	//Clamps "val" to "clamp" if they differ by no more than "diff", used for rounding errors
